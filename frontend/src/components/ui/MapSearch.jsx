...
// Auto-invalidate map size to prevent layout gaps
function MapResize() {
  const map = useMap()
  useEffect(() => {
    map.invalidateSize()
...
// Search bar for city/locality geocoding
...
// Map Action Controller component for Google Maps layer selection & draw tools
  ...
  return (
    <div className="absolute top-3 left-3 z-[1000] flex flex-wrap items-center gap-2 map-control-overlay">
      ...
      <div className="bg-white/95 backdrop-blur-md p-1 rounded-xl shadow-lg border border-gray-200 flex items-center gap-1 text-xs font-semibold text-gray-700">
        <button
          type="button"
          onClick={() => setMapType('google-map')}
          className={`flex items-center gap-1 px-2.5 py-1 rounded-lg transition-all ${
            mapType === 'google-map' ? 'bg-primary-500 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
          }`}
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1 rounded-lg transition-all ${
            mapType === 'google-satellite' ? 'bg-primary-500 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
          }`}
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1 rounded-lg transition-all ${
            mapType === 'google-hybrid' ? 'bg-primary-500 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
          }`}
          title="Google Maps Satellite + Road Overlay"
        >
          <Globe size={13} />
          <span>Hybrid</span>
        </button>
