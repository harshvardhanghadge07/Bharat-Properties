    ...
    <div className="pt-16 min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          ...
          <Link to="/post-property" className="btn-primary"><Plus size={16} /> Post New Property</Link>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl">
            ...
            <Link to="/post-property" className="btn-primary">List Your First Property</Link>
          </div>
        ) : (
            ...
            {properties.map((p) => (
              <div key={p.id} className="bg-white rounded-xl p-4 flex items-center gap-4 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-primary-500 font-bold text-sm">{formatPrice(p.price)}</p>
                ...
                <span className={`tag ${p.status==='ACTIVE'?'bg-green-100 text-green-700':'bg-gray-100 text-gray-600'}`}>{p.status}</span>
                <div className="flex gap-1">
                  <Link to={`/properties/${p.id}`} className="p-2 rounded-lg hover:bg-gray-50 text-gray-500"><ExternalLink size={16} /></Link>
                  <Link to={`/edit-property/${p.id}`} className="p-2 rounded-lg hover:bg-gray-50 text-gray-500"><Pencil size={16} /></Link>
                  <button onClick={() => { if(confirm('Delete this listing?')) deleteMut.mutate(p.id) }}
                    className="p-2 rounded-lg hover:bg-red-50 text-red-500"><Trash2 size={16} /></button>
                </div>
              </div>
            ))}
          </div>
